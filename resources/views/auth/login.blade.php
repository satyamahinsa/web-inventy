...
<body class="bg-gray-100 h-screen flex items-center justify-center">
    ...
    <div class="w-full max-w-sm max-h-sm p-6 sm:p-8 space-y-6 bg-white rounded-lg shadow-lg flex flex-col justify-center items-center">
        ...
        @if (session('status'))
            <div class="status-message text-green-500 mb-4 text-center">
                {{ session('status') }}
            </div>
        @endif
        ...
        <form method="POST" action="{{ route('login') }}" class="w-full space-y-6">
            ...
            @if (Route::has('password.request'))
                <div class="text-right">
                    <a href="{{ route('password.request') }}" class="text-sm sm:text-base text-amber-500 hover:text-amber-700">Lupa Password?</a>
                </div>
            @endif
        ...
        <div class="text-center text-sm sm:text-base">
            <span>Apakah Belum Memiliki Akun? <a href="{{ route('register') }}" class="text-amber-500 hover:text-amber-700">Daftar</a></span>
        </div>
    </div>
