        ...
        @else
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                @foreach ($products as $product)
                    <div class="product-card bg-white p-6 rounded-lg shadow-lg cursor-pointer"
                    ...
                    <div id="product-modal-{{ $product->id }}"
                        ...
                        <div class="bg-white p-6 rounded-lg w-full max-w-2xl relative flex">
                            ...
                            <div class="w-2/3 pl-6 max-h-[500px] overflow-y-auto">
                                <button class="absolute top-2 right-2 text-gray-500 text-2xl"
                                    id="close-modal-{{ $product->id }}">×</button>
                                <h2 class="text-lg font-semibold mb-2">{{ $product->name }}</h2>
                                <p class="text-gray-700">{{ $product->description }}</p>
                                <p class="text-gray-900 font-bold mt-2">Harga:
                                ...
                                <form action="{{ route('products.addToCart', $product->id) }}" method="POST"
                                    ...
                                    <div class="flex items-center mt-4">
                                        ...
                                        <input type="number" name="quantity" min="1"
                                            max="{{ $product->stock }}" value="1" class="border rounded p-2 w-16"
                                            @if ($product->stock == 0) disabled @endif>
                                        <!-- jika stok habis-->
                                        <button type="submit"
                                            class="ml-2 py-2 px-4 rounded flex items-center 
                                            @if ($product->stock == 0) bg-gray-400 text-gray-700 cursor-not-allowed @else bg-blue-500 text-white @endif"
                                            @if ($product->stock == 0) disabled @endif>
