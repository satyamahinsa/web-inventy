<x-app-layout>
    ...
    <div class="container mx-auto my-5 px-4">
        <form action="{{ route('products.index') }}" method="GET" class="flex items-center mb-4" id="searchForm">
            <input type="text" name="search" placeholder="Cari produk..." class="bg-white dark:bg-gray-500 text-gray-800 dark:placeholder:text-white dark:text-white border dark:border-white rounded py-2 px-4 w-32 sm:w-44 md:w-56"
                    ...
                    <div id="product-modal-{{ $product->id }}" class="product-modal fixed inset-0 bg-black bg-opacity-50 hidden justify-center items-center z-50">
                        <div class="bg-white p-6 rounded-lg w-full max-w-2xl relative flex dark:bg-stone-300">
                            ...
                            <div class="w-2/3 pl-6 max-h-[500px] overflow-y-auto ">
                                <button class="absolute top-2 right-2 text-gray-800 text-2xl" id="close-modal-{{ $product->id }}">×</button>
                                <h2 class="text-lg font-bold mb-2">{{ $product->name }}</h2>
                                <p class="text-gray-800">{{ $product->description }}</p>
                                <p class="text-gray-800 font-bold mt-2">Harga: Rp. {{ number_format($product->price, 0, ',', '.') }}</p>
